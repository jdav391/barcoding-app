...
class TestGenerateBarcodeString:
    def test_standard_13_char(self):
        result = generate_barcode_string(unique_id=158404144, sheet_number=3, set_count=7, has_insert=False, is_end_of_group=False)
        assert result == "0307158404144"
        assert len(result) == 13

    def test_end_of_group(self):
        result = generate_barcode_string(unique_id=158404144, sheet_number=7, set_count=7, has_insert=False, is_end_of_group=True)
        assert result == "1707158404144"

    def test_with_insert(self):
        result = generate_barcode_string(unique_id=158404144, sheet_number=1, set_count=1, has_insert=True, is_end_of_group=True)
        assert result == "1111158404144"

    def test_unique_id_zero_padded(self):
        result = generate_barcode_string(unique_id=42, sheet_number=1, set_count=1, has_insert=False, is_end_of_group=True)
        assert result == "1101000000042"

    def test_unique_id_truncated_to_9_digits(self):
        result = generate_barcode_string(unique_id=1234567890, sheet_number=1, set_count=1, has_insert=False, is_end_of_group=True)
        assert result == "1101234567890"
        assert len(result) == 13

    def test_with_divert_14_char(self):
        result = generate_barcode_string(unique_id=158404144, sheet_number=3, set_count=7, has_insert=False, is_end_of_group=False, divert=True)
        assert result == "03071158404144"
        assert len(result) == 14

    def test_divert_false_14_char(self):
        result = generate_barcode_string(unique_id=158404144, sheet_number=3, set_count=7, has_insert=False, is_end_of_group=False, divert=False)
        assert result == "03070158404144"
        assert len(result) == 14

    def test_single_sheet_is_eog(self):
        result = generate_barcode_string(unique_id=1, sheet_number=1, set_count=1, has_insert=False, is_end_of_group=True)
